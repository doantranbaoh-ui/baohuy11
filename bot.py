# -*- coding: utf-8 -*-
# NAO ROBOT - 1500 DÒNG ĐỘT PHÁ AI - 18 GIỌNG BẮC THUẦN VIỆT
# Tác giả: palofsc (palo) | Ngày: 2026-06-24
# Phiên bản: 5.0.0 | Python 3.9+
...
class AIRandomEngine:
    def __init__(self):
        ...
        logger.info("AI Random Engine - Dot pha khoi tao")
    ...
    def _refresh_entropy(self):
        ...
        try:
            with open('/dev/urandom', 'rb') as f: sources.append(f.read(32))
        except: pass
    ...
    def _twist(self):
        for i in range(624):
            ...
            if y % 2 != 0: self.twister_state[i] ^= 0x9908B0DF

    def _mt_random(self) -> int:
        self.counter += 1
        if self.counter >= 624: self._twist(); self.counter = 0
    ...
    def _xor_shift(self, x: int) -> int:
        x ^= (x << 13) & 0xFFFFFFFFFFFFFFFF; x ^= (x >> 7)
        x ^= (x << 17) & 0xFFFFFFFFFFFFFFFF; return x & 0xFFFFFFFFFFFFFFFF
    ...
    def randint(self, min_val: int, max_val: int) -> int:
        ...
        self._refresh_entropy() if self.counter % 100 == 0 else None
        return result

    def choice(self, items: List[Any]) -> Any:
        return items[self.randint(0, len(items) - 1)] if items else None
...
class AIRamManager:
    ...
    def usage_pct(self) -> float: return self.process.memory_info().rss / self.max_bytes
    def usage_mb(self) -> float: return self.process.memory_info().rss / (1024*1024)
    ...
    def clean(self, level: int) -> int:
        ...
        if level >= 1:
            now = time.time()
            for k in [k for k, (v, e) in self.cache.items() if now >= e]: del self.cache[k]
        ...
        self.freed += freed; self.cleans += 1; return freed
    ...
    def ai_clean(self) -> Tuple[int, str]:
        with self.lock:
            ...
            freed = self.clean(lvl); self.last_clean = time.time()
            return freed, act

    def monitor(self):
        while True:
            time.sleep(30)
            if self.usage_pct() >= self.WARNING: self.ai_clean()

    def start(self): Thread(target=self.monitor, daemon=True).start()
...
class Brain:
    ...
    def think(self, uid, txt):
        ...
        elif any(x in txt.lower() for x in ["bot hay","cảm ơn"]): self.mood += 1
        self.mood = max(-10, min(10, self.mood))
        self.state = "aggressive" if self.mood < -5 else "normal"

    def should_reply(self, uid, txt): return uid in self.trusted or ai_random.random() > 0.1
...
GAME_SESSIONS = {}; USED_RIDDLES = defaultdict(list)
member_stats = {"daily_join":defaultdict(int),"daily_leave":defaultdict(int),"total_joined":0,"total_left":0,"current_members":0}

USR_FILE = "usr.json"; BAL_FILE = "balances.json"; DAILY_FILE = "daily_ck.json"
JP_FILE = "jp.json"; STATS_FILE = "stats.json"
...
def load_json(p, d={}):
    ...
    if os.path.exists(p):
        try:
            with open(p,'r') as f: data = json.load(f); ram_mgr.cache_set(f"json_{p}", data, 60); return data
        except: pass
    return d
...
def is_adm(m): return m.from_user.id == ADMIN_ID
...
def deduct_bal(uid, amt):
    if get_bal(uid) >= amt: balance[uid] = get_bal(uid)-amt; save_json(BAL_FILE, {str(k):v for k,v in balance.items()}); return True
    return False
...
def bao_x10(bet: int) -> Tuple[int, bool]:
    if ai_random.random() < 0.10: return bet * 10, True
    return 0, False

# ╔══════════════════════════════════════════════════════════════╗
# ║               18 GIỌNG BẮC THUẦN VIỆT (NO TRUNG/NAM/HUE)   ║
# ╚══════════════════════════════════════════════════════════════╝
...
@dataclass
class VoiceRequest:
    chat_id: int; reply_id: int; text: str; user_name: str; user_id: int
    voice: Optional[Dict] = None; created_at: float = field(default_factory=time.time)
...
def split_tts(text: str) -> List[str]:
    ...
    while len(text) > MAX_CHUNK:
        ...
        for s in seps:
            ...
            if p > MAX_CHUNK//2: best = p+len(s); break
        if best > MAX_CHUNK or best <= MAX_CHUNK//3: best = MAX_CHUNK
        chunks.append(text[:best].strip()); text = text[best:].strip()
    if text: chunks.append(text)
    return chunks

def gen_voice(text: str, speed: float = 1.0) -> Tuple[Optional[BytesIO], int, int, float]:
    clean = re.sub(r'[<>"\'{}|\\^~\[\]`]','',text).strip()
    if not clean: return None, 0, 0, 0
    chunks = split_tts(clean); total = len(chunks); audio = b""; success = 0
    start = time.time()
    for i, c in enumerate(chunks):
        data = fetch_tts(c, speed)
        if data: audio += data; success += 1
        if i < total - 1: time.sleep(0.1)
    gen_time = time.time() - start
    if not audio or len(audio) < 100: return None, success, total, gen_time
...
def voice_worker():
    while True:
        try:
            ...
            if not txt: voice_queue.task_done(); continue
            ...
            try: bot.delete_message(req.chat_id, status.message_id)
            except: pass
            if audio and isinstance(audio, BytesIO) and audio.getbuffer().nbytes > 100:
                ...
                try: bot.send_voice(req.chat_id, audio, reply_to_message_id=req.reply_id, caption=cap, parse_mode="HTML"); brain.stats["voice"] += 1
                except:
                    try: audio.seek(0); bot.send_audio(req.chat_id, audio, reply_to_message_id=req.reply_id, caption=cap, parse_mode="HTML", title=f"Voice - {v['name']}"); brain.stats["voice"] += 1
                    except: bot.send_message(req.chat_id, f"❌ Lỗi gửi audio.", reply_to_message_id=req.reply_id)
            else:
...
def init_game(uid, gt):
    ...
    if gt=="taixiu": return {"type":"taixiu",**bases}
    if gt=="baucua": return {"type":"baucua","sym":["Cua","Ca","Tom","Ga","Nai","Bau"],**bases}
    if gt=="kbb": return {"type":"kbb","score":0,"bot":0,"draw":0}
    if gt=="doanso": return {"type":"doanso","secret":ai_random.randint(1,100),"att":0,"max":7}
    if gt=="lxn": return {"type":"lxn",**bases}
    if gt=="xx": return {"type":"xx",**bases}
    if gt=="caudo": return {"type":"caudo","score":0,"qnum":0,"cur":None,"hint":False,"ans":False,"start":0}
    if gt=="chanle": return {"type":"chanle",**bases}
    if gt=="caothap": return {"type":"caothap",**bases}
    if gt=="doanso2": return {"type":"doanso2","secret":ai_random.randint(1,100),"att":0,"max":5}
    if gt=="keo": return {"type":"keo","bal":1000,"w":0,"l":0}
    if gt=="bingo": return {"type":"bingo","bal":1000,"w":0,"l":0}
    return {}
...
def taixiu(m):
    ...
    if len(parts)<3:
        ...
        g=GAME_SESSIONS[uid]; bot.reply_to(m,f"TAI XIU BAO X10\n/taixiu [tai/xiu] [cuoc]\nGame: {g['bal']} xu"); return
    ch,bt=parts[1].lower(),0
    try: bt=int(parts[2])
    except: bot.reply_to(m,"Cuoc so"); return
    if ch not in['tai','xiu']: bot.reply_to(m,"tai/xiu"); return
    if uid not in GAME_SESSIONS: GAME_SESSIONS[uid]=init_game(uid,"taixiu")
    g=GAME_SESSIONS[uid]
    if bt>g["bal"] or bt<1: bot.reply_to(m,f"So du: {g['bal']}"); return
    dice=[ai_random.randint(1,6) for _ in range(3)]; total=sum(dice); res="tai" if total>=11 else "xiu"
    ds=" ".join("⚀⚁⚂⚃⚄⚅"[d-1] for d in dice)
    bao_bonus,is_bao=bao_x10(bt)
    if ch==res: win=bt*3+bao_bonus; g["bal"]+=win; g["w"]+=1; out=f"Thang +{win}" + (" BAO X10!!!" if is_bao else "")
    else: g["bal"]-=bt; g["l"]+=1; out=f"Thua -{bt}"
    brain.stats["games"]+=1
    m2=bot.reply_to(m,f"TAI XIU: {ds} = {total} -> {res.upper()}\n{out} | Game: {g['bal']} xu"); del_both(m,m2.message_id)
...
def doanso(m):
    ...
    if gs==g["secret"]:
        ...
        add_bal(uid,rw); m2=bot.reply_to(m,f"CHINH XAC! So {g['secret']} ({g['att']} lan)\n+{rw}" + (" BAO!!!" if is_bao else "")); del GAME_SESSIONS[uid]
...
# ─── CÂU ĐỐ AI ──────────────────────────────────────────────────────────
CAUDO_LIST = [
    {"q":"Có đàn chuột điếc đi qua cầu, mấy con?","a":["24","hai tư"],"h":"Điếc = hư tai"},
...
def caudo(m):
    ...
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type")!="caudo" or GAME_SESSIONS[uid].get("ans",False):
        ...
        GAME_SESSIONS[uid]={"type":"caudo","score":GAME_SESSIONS[uid].get("score",0) if uid in GAME_SESSIONS else 0,"qnum":GAME_SESSIONS[uid].get("qnum",0)+1 if uid in GAME_SESSIONS else 1,"cur":r,"hint":False,"ans":False,"start":time.time()}
        m2=bot.reply_to(m,f"CAU DO #{GAME_SESSIONS[uid]['qnum']}\n60s\n{r['q']}\n/caudo [dap an]"); del_both(m,m2.message_id)
        def timeout():
            time.sleep(60)
            if uid in GAME_SESSIONS and not GAME_SESSIONS[uid].get("ans",True): GAME_SESSIONS[uid]["ans"]=True; bot.send_message(m.chat.id,f"Het gio! Dap an: {r['a'][0]}")
...
def chanle(m):
    ...
    if ch not in['chan','le']: bot.reply_to(m,"chan/le"); return
    if uid not in GAME_SESSIONS: GAME_SESSIONS[uid]=init_game(uid,"chanle")
    g=GAME_SESSIONS[uid]
    if bt>g["bal"] or bt<1: bot.reply_to(m,f"So du: {g['bal']}"); return
...
def caothap(m):
    ...
    if ch not in['cao','thap']: bot.reply_to(m,"cao/thap"); return
    if uid not in GAME_SESSIONS: GAME_SESSIONS[uid]=init_game(uid,"caothap")
    g=GAME_SESSIONS[uid]
    if bt>g["bal"] or bt<1: bot.reply_to(m,f"So du: {g['bal']}"); return
...
def doanso2(m):
    ...
    if gs==g["secret"]:
        ...
        add_bal(uid,rw); m2=bot.reply_to(m,f"CHINH XAC! +{rw}" + (" BAO!!!" if is_bao else "")); del GAME_SESSIONS[uid]
    elif g["att"]>=g["max"]: m2=bot.reply_to(m,f"HET LUOT! So {g['secret']}"); del GAME_SESSIONS[uid]
    elif gs<g["secret"]: m2=bot.reply_to(m,f"CAO HON ({g['max']-g['att']})")
    else: m2=bot.reply_to(m,f"THAP HON ({g['max']-g['att']})")
    del_both(m,m2.message_id)
...
def nohu_cmd(m):
    ...
    total=bet+nohu_fee
    if not deduct_bal(uid,total): bot.reply_to(m,f"Khong du! Can {total}"); return
    nohu_jp+=int(bet*nohu_mult)
    bao_bonus,is_bao=bao_x10(bet)
    c1,c2,c3=[ai_random.choice(["🍒","🍋","🍊","🍇","💎","🔔","7️⃣"]) for _ in range(3)]
    if c1==c2==c3:
        if c1=="7️⃣": win=nohu_jp+bao_bonus; add_bal(uid,win); nohu_hist.append({"name":m.from_user.first_name,"amount":win}); nohu_jp=100000; out=f"JACKPOT! +{win}" + (" BAO!!!" if is_bao else "")
        else: win=bet*5+bao_bonus; add_bal(uid,win); out=f"No! +{win}" + (" BAO!!!" if is_bao else "")
    elif c1==c2 or c2==c3 or c1==c3: win=int(bet*0.5); add_bal(uid,win); out=f"Hoan {win}"
    else: out=f"Thua -{total}"
    brain.stats["nohu"]+=1
    m2=bot.reply_to(m,f"{c1}{c2}{c3}\n{out}\nSo du: {get_bal(uid)} xu"); del_both(m,m2.message_id)

@bot.message_handler(commands=['daily'])
def daily(m):
    if not is_grp(m): return
    uid=m.from_user.id; today=date.today().isoformat()
    if daily_ck.get(uid)==today: bot.reply_to(m,f"Da diem danh\nSo du: {get_bal(uid)} xu"); return
    daily_ck[uid]=today; rw=500+ai_random.randint(0,1000); add_bal(uid,rw)
...
def balance_cmd(m):
    ...
    m2=bot.reply_to(m,f"{n}: {get_bal(t):,} xu"); del_both(m,m2.message_id)
...
def top(m):
    ...
    text="BANG XEP HANG:\n"
    for i,(uid,bal) in enumerate(sb): text+=f"{i+1}. {users.get(str(uid),str(uid))}: {bal:,} xu\n"
...
def give(m):
    ...
    if not deduct_bal(uid,amt+int(amt*0.05)): bot.reply_to(m,"Khong du xu"); return
    add_bal(target,amt); m2=bot.reply_to(m,f"Da chuyen {amt} xu"); del_both(m,m2.message_id)
        ...
        for _ in range(len(AI_KEYS)):
            ...
            try:
                ...
                if r.status_code==200:
                    ...
                    k["fail"]=0; mem.append(prompt); mem.append(txt); brain.stats["ai"]+=1; return txt
                else: k["fail"]+=1
            except: k["fail"]+=1
...
def start(m):
    if not is_grp(m): return
    users[str(m.from_user.id)]=m.from_user.first_name; save_json(USR_FILE,users)
    brain.trusted.add(m.from_user.id)
    help_text=("NAO ROBOT - DOT PHA AI\n12 GAMES BAO X10\n/taixiu /baucua /kbb /doanso /lxn /xx /caudo /chanle /caothap /keo /bingo /doanso2\n/nohu | /daily /balance /top /give\n/voice [text] - 18 giong Bac thuần Viet\n/ban /mute /unmute /warn /stats")
    m2=bot.reply_to(m,help_text); del_both(m,m2.message_id)
...
def voice_cmd(m):
    ...
    if not txt:
        vl="18 GIONG BAC THUAN VIET:\n"
        for i,v in enumerate(VOICE_LIST,1): vl+=f"{i}. {v['emoji']} {v['name']} (x{v['speed']})\n"
        vl+="/voice [text] - Random | /voice [so] [text] - Chon"
    ...
    try: vqueue.put_nowait(req); m2=bot.reply_to(m,"🎙️ Dang tao giong noi..."); auto_del(m.chat.id,m2.message_id,10)
...
def warn(m):
    ...
    target,reason=extract_user_and_reason(m,bot.get_me().username)
    if not target: return
    warns[target]=warns.get(target,0)+1; cnt=warns[target]; bot.delete_message(m.chat.id,m.message_id)
    if cnt>=3:
        try: bot.ban_chat_member(m.chat.id,target,until_date=int(time.time())+3600); del warns[target]
        except: pass
...
def stats(m):
    ...
    m2=bot.reply_to(m,f"THONG KE\nThanh vien: {rc}"); del_both(m,m2.message_id)

@bot.message_handler(commands=['ramstatus'])
def ramstatus(m):
    if not is_grp(m): return
    m2=bot.reply_to(m,f"RAM: {ram_mgr.usage_mb():.1f}MB | Clean: {ram_mgr.cleans} lan | Freed: {ram_mgr.freed/1024/1024:.1f}MB"); del_both(m,m2.message_id)
...
def welcome(m):
    ...
    for u in m.new_chat_members:
        ...
        users[str(u.id)]=u.first_name; save_json(USR_FILE,users)
        member_stats["daily_join"][date.today().isoformat()]+=1; member_stats["total_joined"]+=1; member_stats["current_members"]+=1
        m2=bot.send_message(m.chat.id,f"Chao mung {u.first_name}!"); auto_del(m.chat.id,m2.message_id)
...
def goodbye(m):
    ...
    member_stats["daily_leave"][date.today().isoformat()]+=1; member_stats["total_left"]+=1; member_stats["current_members"]=max(0,member_stats["current_members"]-1)
    ...
    while True:
        try:
            ...
            if now.minute==0 and now.hour!=last and users:
                ...
                auto_del(GROUP_ID,msg.message_id); last=now.hour
            if now.minute!=0: last=-1
...
def auto_save():
    while True:
        time.sleep(600)
        try: save_json(USR_FILE,users); save_json(BAL_FILE,{str(k):v for k,v in balance.items()}); save_json(DAILY_FILE,daily_ck); save_json(JP_FILE,{"jp":nohu_jp})
        except: pass
...
def main():
    ...
    ram_mgr.start()
    logger.info(f"NAO ROBOT DOT PHA AI KHOI DONG\nUsers: {len(users)} | JP: {nohu_jp:,}\n18 Giong Bac Thuần Viet | 12 Games Bao X10 | AI Random Sieu Kho")
