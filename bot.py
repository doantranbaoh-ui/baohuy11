# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║                    NAO ROBOT v5.1.0 - COMPLETE REWRITE                      ║
# ║                    FIX ALL GAMES + STABILITY + PERFORMANCE                  ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
def init_game(uid, gt):
    ...
    if gt == "taixiu":
        return {"type": "taixiu", "w": 0, "l": 0, **bases}
    if gt == "baucua":
        return {"type": "baucua", "sym": ["Cua", "Ca", "Tom", "Ga", "Nai", "Bau"], "w": 0, "l": 0, **bases}
    if gt == "kbb":
        return {"type": "kbb", "score": 0, "bot": 0, "draw": 0, **bases}
    if gt == "doanso":
        return {"type": "doanso", "secret": ai_random.randint(1, 100), "att": 0, "max": 7, **bases}
    if gt == "lxn":
        return {"type": "lxn", "w": 0, "l": 0, **bases}
    if gt == "xx":
        return {"type": "xx", "w": 0, "l": 0, **bases}
    if gt == "caudo":
        return {"type": "caudo", "score": 0, "qnum": 0, "cur": None, "hint": False, "ans": False, **bases}
    if gt == "chanle":
        return {"type": "chanle", "w": 0, "l": 0, **bases}
    if gt == "caothap":
        return {"type": "caothap", "w": 0, "l": 0, **bases}
    if gt == "doanso2":
        return {"type": "doanso2", "secret": ai_random.randint(1, 100), "att": 0, "max": 5, **bases}
    if gt == "keo":
        return {"type": "keo", "w": 0, "l": 0, **bases}
    if gt == "bingo":
        return {"type": "bingo", "w": 0, "l": 0, **bases}
    return {"type": gt, **bases}
...
def taixiu(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> 3 xúc xắc, Tổng 11-18 = TÀI, 3-10 = XỈU\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10 tiền thưởng\n"
            f"💰 <b>Thắng:</b> x3 tiền cược\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if ch not in ['tai', 'xiu']:
        m2 = bot.reply_to(m, "❌ Chọn <b>tai</b> hoặc <b>xiu</b>")
        del_both(m, m2.message_id)
        return
    ...
    _, _, out = resolve_bet(uid, bt, won, multiplier=3)
    if won:
        g["w"] += 1
    else:
        g["l"] += 1
    ...
    m2 = bot.reply_to(m,
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 {ds}\n"
        f"📊 Tổng: <b>{total}</b> → <b>{'TÀI' if total >= 11 else 'XỈU'}</b>\n"
...
def baucua(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> 3 xúc xắc, chọn 1 trong 6 linh vật\n"
            f"🐄 Bầu | 🦀 Cua | 🐟 Cá | 🐔 Gà | 🦌 Nai | 🦐 Tôm\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Thưởng:</b> x3 mỗi con trúng\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /baucua [bau/cua/ca/tom/ga/nai] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"🏆 Thắng: {g['w']} | Thua: {g['l']}\n"
            f"💰 Số dư: {get_bal(uid):,} xu",
            parse_mode="HTML"
    ...
    if ch not in sm:
        m2 = bot.reply_to(m, "❌ Chọn: bau, cua, ca, tom, ga, nai")
        del_both(m, m2.message_id)
        return
    ...
    if match > 0:
        _, _, out = resolve_bet(uid, bt, True, multiplier=match * 3)
        g["w"] += 1
    else:
    ...
    m2 = bot.reply_to(m,
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 {' | '.join(rs)}\n"
        f"📊 Trúng <b>{match}</b> con <b>{ch.upper()}</b>\n"
        f"{out}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🏆 Thắng: {g['w']} | Thua: {g['l']}\n"
        f"💰 Số dư: <b>{get_bal(uid):,} xu</b>",
...
def kbb(m):
    ...
    uid = m.from_user.id
    parts = m.text.split()
    chs = {"keo": "✌️ Kéo", "bua": "✊ Búa", "bao": "✋ Bao"}
    EMOJI_MAP = {"keo": "✌️", "bua": "✊", "bao": "✋"}
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Kéo thắng Bao, Bao thắng Búa, Búa thắng Kéo\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Thắng:</b> x2 tiền cược | <b>Hòa:</b> hoàn tiền\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    uc = chs[ch]
    bot_choice = ai_random.choice(list(chs.keys()))
    bc = chs[bot_choice]
    ui, bi = list(chs.keys()).index(ch), list(chs.keys()).index(bot_choice)

    if ui == bi:
        add_bal(uid, bt)
        r = "🤝 Hòa! Hoàn lại tiền cược"
        g["draw"] += 1
    elif (ui == 0 and bi == 2) or (ui == 1 and bi == 0) or (ui == 2 and bi == 1):
        _, _, out = resolve_bet(uid, bt, True, multiplier=2)
        r = out
        g["score"] += 1
    else:
        _, _, out = resolve_bet(uid, bt, False)
    ...
    m2 = bot.reply_to(m,
        f"✊ <b>KÉO BÚA BAO</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 Bạn: {EMOJI_MAP[ch]} <b>{uc}</b>\n"
        f"🤖 Bot: {EMOJI_MAP[bot_choice]} <b>{bc}</b>\n"
        f"{r}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🏆 Bạn: {g['score']} | 🤖 Bot: {g['bot']} | 🤝 Hòa: {g['draw']}\n"
        f"💰 Số dư: <b>{get_bal(uid):,} xu</b>",
...
def doanso(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            f"🔢 <b>ĐOÁN SỐ BÃO X10</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Đoán số 1-100, tối đa 7 lần\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Thưởng:</b> Càng ít lần đoán càng nhiều xu\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /doanso [số] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"💰 Số dư: {get_bal(uid):,} xu",
    ...
    if gs == g["secret"]:
        multiplier = (8 - g["att"]) * 2
        _, _, out = resolve_bet(uid, bt, True, multiplier=max(multiplier, 1))
        m2 = bot.reply_to(m,
            f"🎉 <b>CHÍNH XÁC!</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 Số: <b>{g['secret']}</b>\n"
            f"🔢 Lần đoán: <b>{g['att']}</b>\n"
            f"{out}\n"
            f"💰 Số dư: <b>{get_bal(uid):,} xu</b>",
            parse_mode="HTML"
        )
        ...
        m2 = bot.reply_to(m, f"📈 <b>CAO HƠN!</b> Còn {g['max'] - g['att']} lần đoán")
    else:
        m2 = bot.reply_to(m, f"📉 <b>THẤP HƠN!</b> Còn {g['max'] - g['att']} lần đoán")
    del_both(m, m2.message_id)
...
def lxn(m):
    ...
    if len(parts) < 3:
        ...
        g["last_active"] = time.time()
        m2 = bot.reply_to(m,
            f"🎲 <b>LẮC XI NGẦU BÃO X10</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Đoán tổng 3 xúc xắc (3-18)\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Trúng chính xác:</b> x10 | <b>Lệch 1:</b> hoàn 50%\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /lxn [tổng 3-18] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"🏆 Thắng: {g['w']} | Thua: {g['l']}\n"
            f"💰 Số dư: {get_bal(uid):,} xu",
            parse_mode="HTML"
    ...
    if gt < 3 or gt > 18:
        m2 = bot.reply_to(m, "❌ Tổng từ 3-18.")
        del_both(m, m2.message_id)
        return
    ...
    if total == gt:
        _, _, out = resolve_bet(uid, bt, True, multiplier=10)
        g["w"] += 1
    elif abs(total - gt) == 1:
        refund = int(bt * 0.5)
        add_bal(uid, refund)
        out = f"🤏 Gần đúng! Hoàn {refund:,} xu"
    else:
        _, _, out = resolve_bet(uid, bt, False)
        g["l"] += 1
...
def xx(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Đoán mặt xúc xắc (1-6)\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Trúng:</b> x4 | <b>Lệch 1:</b> hoàn 50%\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /xx [số 1-6] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"🏆 Thắng: {g['w']} | Thua: {g['l']}\n"
    ...
    if gs == dr:
        _, _, out = resolve_bet(uid, bt, True, multiplier=4)
        g["w"] += 1
    elif abs(gs - dr) == 1:
        refund = int(bt * 0.5)
        add_bal(uid, refund)
...
def caudo(m):
    ...
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "caudo" or GAME_SESSIONS[uid].get("ans", False):
        ...
        GAME_SESSIONS[uid] = {
            "type": "caudo", "score": old_score, "qnum": old_qnum,
            "cur": r, "hint": False, "ans": False,
            "start": time.time(), "last_active": time.time()
        }
        m2 = bot.reply_to(m,
            f"🧩 <b>CÂU ĐỐ #{GAME_SESSIONS[uid]['qnum']}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⏱️ 60s\n"
            f"📝 {r['q']}\n\n"
            f"/caudo [đáp án]\n"
            f"/caudo hint (gợi ý -1đ)",
            parse_mode="HTML"
        )
        del_both(m, m2.message_id)
...
def chanle(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Bot random số 1-100, bạn đoán chẵn/lẻ\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Thắng:</b> x3 tiền cược\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
...
def caothap(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Bot random số 1-100, >50 = CAO, ≤50 = THẤP\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Thắng:</b> x3 tiền cược\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if ch not in ['cao', 'thap']:
        m2 = bot.reply_to(m, "❌ Chọn <b>cao</b> hoặc <b>thap</b>")
        del_both(m, m2.message_id)
        return
...
def keo(m):
    ...
    if len(parts) < 3:
        ...
        g["last_active"] = time.time()
        m2 = bot.reply_to(m,
            f"🎰 <b>KÉO LUI BÃO X10</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Đoán số 0-9\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Trúng:</b> x5 tiền cược\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /keo [0-9] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"🏆 Thắng: {g['w']} | Thua: {g['l']}\n"
...
def bingo(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            f"🎱 <b>BINGO BÃO X10</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Đoán số 1-36\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Trúng:</b> x8 tiền cược\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /bingo [1-36] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"🏆 Thắng: {g['w']} | Thua: {g['l']}\n"
    ...
    _, _, out = resolve_bet(uid, bt, won, multiplier=8)
    if won:
        g["w"] += 1
    else:
        g["l"] += 1
...
def doanso2(m):
    ...
    if len(parts) < 3:
        ...
        m2 = bot.reply_to(m,
            f"⚡ <b>ĐOÁN SỐ SIÊU TỐC BÃO X10</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🎯 <b>Luật:</b> Đoán số 1-100, tối đa 5 lần\n"
            f"💥 <b>Bão X10:</b> 10% cơ hội nhân 10\n"
            f"💰 <b>Thưởng:</b> Càng ít lần đoán càng nhiều xu (x5-x35)\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"📝 /doanso2 [số] [cược]\n"
            f"💵 Cược: {GAME_MIN_BET:,} - {GAME_MAX_BET:,} xu\n"
            f"💰 Số dư: {get_bal(uid):,} xu",
...
def ask_ai(prompt):
    ...
    if not acquired:
        logger.warning("AI lock timeout - fallback")
...
def start(m):
    ...
    help_text = (
        ...
        "📊 /stats | 💾 /ramstatus\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🛠️ Stable v5.1.0 - All fixes applied"
    )
...
def chat(m):
    ...
    def _ai():
        try:
            reply = ask_ai(m.text)
            if f"@{bot.get_me().username}" in m.text or \
...
def welcome(m):
    ...
    for u in m.new_chat_members:
        ...
        m2 = bot.send_message(
            m.chat.id,
            f"👋 Chào mừng {html.escape(u.first_name)}!\n"
            f"🤖 Xem lệnh: /start"
        )
        auto_del(m.chat.id, m2.message_id)
            ...
            if now.minute == 0 and now.hour != last and users:
                ...
                try:
                    msg = bot.send_message(
                        GROUP_ID,
                        f"🕐 {now.strftime('%H:%M')} | {un}... {ai_random.choice(get_kho())}"
                    )
...
def main():
    ...
    logger.info("="*60)
    logger.info("NAO ROBOT v5.1.0 STARTING...")
    ...
    logger.info(f"12 Games Bão X10 | 18 Giọng Bắc | AI Chat")
    logger.info(f"All fixes applied - Stable v5.1.0")
    logger.info("="*60)
