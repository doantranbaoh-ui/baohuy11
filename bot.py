...
from keep_alive import keep_alive  # Web server chống sleep
...
async def nap(msg: types.Message):
    ...
    nap_requests[uid] = current_time  # lưu thời gian tạo lệnh
...
async def buy(msg: types.Message):
    ...
    save_sold_acc(acc)  # lưu acc đã bán
...
async def bill(msg: types.Message):
    ...
    # Check thời gian tạo lệnh /nap
    if uid not in nap_requests:
        return await msg.answer("❌ Bạn chưa tạo lệnh /nap hoặc lệnh đã hết hạn. Gõ /nap để tạo lại.")

    elapsed = current_time - nap_requests[uid]
    if elapsed > 20 * 60:  # 20 phút = 1200 giây
        del nap_requests[uid]  # xóa lệnh cũ
        return await msg.answer("❌ Lệnh nạp đã quá 20 phút, vui lòng tạo lại lệnh /nap.")

    # Nếu trong 20 phút → gửi bill cho admin
