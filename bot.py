# -*- coding: utf-8 -*-
# NAO ROBOT - 2000+ DÒNG FULL AI - RENDER READY
# Tác giả: palofsc (palo) | Ngày: 2026-06-24
# Phiên bản: 2.0.0 | Python 3.9+
# Tính năng: AI Brain + AI RAM + AI No Hu + AI Cau Do + 12 Games Bao X10
#            + 18 Voice + Auto Delete + Quan li nhom + Tai chinh + Thong ke
        ...
        with self.clean_lock:
            ...
            try:
                ...
                if usage_pct >= self.CRITICAL:
                    ...
                    logger.warning(f"RAM AGGRESSIVE: {current_mb:.1f}MB ({usage_pct*100:.1f}%)")
    ...
    def health_check(self) -> str:
        ...
        if now - self.last_health_check > 300:
            ...
            if self.stats["errors"] > 20:
                ...
                self.stats["errors"] = 0
                logger.warning("Brain entering repair mode")
                return "repair"
            self.save_state()
        return "ok"
...
VOICE_LIST: List[Dict[str, Any]] = [
    {"name": f"Giong {i+1}", "lang": "vi", "speed": 0.5 + i * 0.07}
    for i in range(18)
]
...
def voice_worker():
    while True:
        try:
            ...
            except Exception as gen_err: logger.error(f"Generate voice error: {gen_err}")
            if audio and isinstance(audio, BytesIO) and audio.getbuffer().nbytes > 100:
                audio.name = f"voice_{int(time.time())}.mp3"
                cap = f"Giong noi: {voice['name']}"
                try:
                    bot.send_voice(req.chat_id, audio, reply_to_message_id=req.reply_id, caption=cap)
                    brain.stats["voice_generated"] += 1
                except Exception:
                    try:
                        audio.seek(0)
                        bot.send_audio(req.chat_id, audio, reply_to_message_id=req.reply_id, title="Voice", caption=cap)
                        brain.stats["voice_generated"] += 1
                    except Exception as send_err:
                        logger.error(f"Send audio error: {send_err}")
                        try: bot.send_message(req.chat_id, f"Loi gui audio.", reply_to_message_id=req.reply_id)
                        except: pass
            else:
...
def taixiu_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]
        msg = bot.reply_to(m, f"TAI XIU BAO X10\n/taixiu [tai/xiu] [cuoc]\nGame: {g['bal']} xu\nThang: x3 | Bao: x10!")
    ...
    if ch == res:
        ...
        out = f"Thang +{win} xu" + (" BAO X10!!!" if is_bao else "")
    else:
...
def baucua_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]
        msg = bot.reply_to(m, f"BAU CUA BAO X10\n/baucua [con] [cuoc]\nGame: {g['bal']} xu"); del_both(m, msg.message_id); return
    ch, bt = parts[1].lower(), 0
    try: bt = int(parts[2])
    except: msg = bot.reply_to(m, "Cuoc phai la so."); del_both(m, msg.message_id); return
    if ch not in sm: msg = bot.reply_to(m, f"Chon: {','.join(sm)}"); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "baucua":
        GAME_SESSIONS[uid] = init_game(uid, "baucua")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    bao_bonus, is_bao = bao_x10(bt)
    if match > 0: win = bt * match * 3 + bao_bonus; g["bal"] += win; g["w"] += 1; out = f"Thang +{win} xu" + (" BAO!!!" if is_bao else "")
    else: g["bal"] -= bt; g["l"] += 1; out = f"Thua -{bt} xu"
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"BAU CUA: {' '.join(rs)}\nTrung {match} con\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 3: KÉO BÚA BAO + BÃO X10 ──────────────────────────────────────
...
def kbb_cmd(m):
    ...
    if len(parts) < 2:
        ...
        g = GAME_SESSIONS[uid]; msg = bot.reply_to(m, f"KEO BUA BAO X10\n/kbb [keo/bua/bao]\nBan: {g['score']} | Bot: {g['bot']}")
        del_both(m, msg.message_id); return
    ch = parts[1].lower()
    if ch not in chs: msg = bot.reply_to(m, "Chon keo/bua/bao"); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "kbb":
        GAME_SESSIONS[uid] = init_game(uid, "kbb")
    ...
    if ui == bi: r = "Hoa"; g["draw"] += 1
        ...
        pts = 3 + bao_bonus; g["score"] += pts; r = f"Thang +{pts}" + (" BAO!!!" if is_bao else "")
    else: r = "Thua"; g["bot"] += 1
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"{uc} vs {bc}\n{r}\nBan: {g['score']} | Bot: {g['bot']} | Hoa: {g['draw']}")
    del_both(m, msg.message_id)

# ─── GAME 4: ĐOÁN SỐ + BÃO X10 ──────────────────────────────────────────
...
def doanso_cmd(m):
    ...
    if len(parts) < 2:
        ...
        msg = bot.reply_to(m, "DOAN SO BAO X10\n/doanso [so] (1-100)\n7 lan doan!"); del_both(m, msg.message_id); return
    try: gs = int(parts[1])
    except: msg = bot.reply_to(m, "Nhap so 1-100."); del_both(m, msg.message_id); return
    if gs < 1 or gs > 100: msg = bot.reply_to(m, "1-100."); del_both(m, msg.message_id); return
    ...
    if gs == g["secret"]:
        ...
        add_balance(uid, rw)
        msg = bot.reply_to(m, f"CHINH XAC! So {g['secret']} ({g['att']} lan)\n+{rw} xu" + (" BAO!!!" if is_bao else ""))
        del GAME_SESSIONS[uid]
    elif g["att"] >= g["max"]: msg = bot.reply_to(m, f"HET LUOT! So la {g['secret']}."); del GAME_SESSIONS[uid]
    elif gs < g["secret"]: msg = bot.reply_to(m, f"{gs} -> CAO HON ({g['max']-g['att']} lan)")
    else: msg = bot.reply_to(m, f"{gs} -> THAP HON ({g['max']-g['att']} lan)")
    del_both(m, msg.message_id)

# ─── GAME 5: LẮC XÍ NGẦU + BÃO X10 ──────────────────────────────────────
...
def lxn_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]
        msg = bot.reply_to(m, f"LAC XI NGAU BAO X10\n/lxn [tong 3-18] [cuoc]\nGame: {g['bal']} xu\nTrung chinh xac: x10!")
        del_both(m, msg.message_id); return
    try: gt, bt = int(parts[1]), int(parts[2])
    except: msg = bot.reply_to(m, "/lxn [tong 3-18] [cuoc]"); del_both(m, msg.message_id); return
    if gt < 3 or gt > 18: msg = bot.reply_to(m, "3-18."); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "lxn":
        GAME_SESSIONS[uid] = init_game(uid, "lxn")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    ds = " ".join("⚀⚁⚂⚃⚄⚅"[d-1] for d in dice)
    bao_bonus, is_bao = bao_x10(bt)
    if total == gt: win = bt * 10 + bao_bonus; g["bal"] += win; out = f"CHINH XAC! +{win} xu" + (" BAO!!!" if is_bao else "")
    elif abs(total - gt) == 1: win = int(bt * 0.5); g["bal"] += win; out = f"Gan dung! Hoan {win} xu"
    else: g["bal"] -= bt; out = f"Thua -{bt} xu"
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"LAC XI NGAU: {ds} = {total}\nDoan: {gt}\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 6: XÚC XẮC + BÃO X10 ──────────────────────────────────────────
...
def xx_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]
        msg = bot.reply_to(m, f"XUC XAC BAO X10\n/xx [so 1-6] [cuoc]\nGame: {g['bal']} xu\nTrung: x4!")
        del_both(m, msg.message_id); return
    try: gs, bt = int(parts[1]), int(parts[2])
    except: msg = bot.reply_to(m, "/xx [so 1-6] [cuoc]"); del_both(m, msg.message_id); return
    if gs < 1 or gs > 6: msg = bot.reply_to(m, "1-6."); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "xx":
        GAME_SESSIONS[uid] = init_game(uid, "xx")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    bao_bonus, is_bao = bao_x10(bt)
    if gs == dr: win = bt * 4 + bao_bonus; g["bal"] += win; out = f"TRUNG! +{win} xu" + (" BAO!!!" if is_bao else "")
    elif abs(gs - dr) == 1: win = int(bt * 0.5); g["bal"] += win; out = f"Gan dung! Hoan {win} xu"
    else: g["bal"] -= bt; out = f"Thua -{bt} xu"
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"XUC XAC: {de} {dr}\nDoan: {gs}\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 7: CÂU ĐỐ AI + BÃO X10 ────────────────────────────────────────
    ...
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "caudo" or GAME_SESSIONS[uid].get("ans", False):
        ...
        def timeout():
            ...
            if uid in GAME_SESSIONS and not GAME_SESSIONS[uid].get("ans", True):
                ...
                bot.send_message(m.chat.id, f"Het gio! Dap an: {riddle['a'][0]}")
...
def chanle_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]; msg = bot.reply_to(m, f"CHAN LE BAO X10\n/chanle [chan/le] [cuoc]\nGame: {g['bal']} xu"); del_both(m, msg.message_id); return
    ch, bt = parts[1].lower(), 0
    try: bt = int(parts[2])
    except: msg = bot.reply_to(m, "Cuoc phai la so."); del_both(m, msg.message_id); return
    if ch not in ['chan', 'le']: msg = bot.reply_to(m, "Chon chan/le"); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "chanle":
        GAME_SESSIONS[uid] = init_game(uid, "chanle")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    bao_bonus, is_bao = bao_x10(bt)
    if ch == res: win = bt * 3 + bao_bonus; g["bal"] += win; g["w"] += 1; out = f"Thang +{win} xu" + (" BAO!!!" if is_bao else "")
    else: g["bal"] -= bt; g["l"] += 1; out = f"Thua -{bt} xu"
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"CHAN LE: So {num} -> {res.upper()}\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 9: CAO THẤP + BÃO X10 ─────────────────────────────────────────
...
def caothap_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]; msg = bot.reply_to(m, f"CAO THAP BAO X10\n/caothap [cao/thap] [cuoc]\nGame: {g['bal']} xu"); del_both(m, msg.message_id); return
    ch, bt = parts[1].lower(), 0
    try: bt = int(parts[2])
    except: msg = bot.reply_to(m, "Cuoc phai la so."); del_both(m, msg.message_id); return
    if ch not in ['cao', 'thap']: msg = bot.reply_to(m, "Chon cao/thap"); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "caothap":
        GAME_SESSIONS[uid] = init_game(uid, "caothap")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"CAO THAP: So {num} -> {res.upper()}\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 10: KÉO LỤI + BÃO X10 ─────────────────────────────────────────
...
def keo_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]; msg = bot.reply_to(m, f"KEO LUI BAO X10\n/keo [0-9] [cuoc]\nGame: {g['bal']} xu\nTrung: x5!"); del_both(m, msg.message_id); return
    try: gs, bt = int(parts[1]), int(parts[2])
    except: msg = bot.reply_to(m, "/keo [so 0-9] [cuoc]"); del_both(m, msg.message_id); return
    if gs < 0 or gs > 9: msg = bot.reply_to(m, "0-9."); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "keo":
        GAME_SESSIONS[uid] = init_game(uid, "keo")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    bao_bonus, is_bao = bao_x10(bt)
    if gs == result: win = bt * 5 + bao_bonus; g["bal"] += win; out = f"TRUNG! +{win} xu" + (" BAO!!!" if is_bao else "")
    else: g["bal"] -= bt; out = f"Thua -{bt} xu"
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"KEO LUI: So {result}\nDoan: {gs}\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 11: BINGO + BÃO X10 ───────────────────────────────────────────
...
def bingo_cmd(m):
    ...
    if len(parts) < 3:
        ...
        g = GAME_SESSIONS[uid]; msg = bot.reply_to(m, f"BINGO BAO X10\n/bingo [1-36] [cuoc]\nGame: {g['bal']} xu\nTrung: x8!"); del_both(m, msg.message_id); return
    try: gs, bt = int(parts[1]), int(parts[2])
    except: msg = bot.reply_to(m, "/bingo [so 1-36] [cuoc]"); del_both(m, msg.message_id); return
    if gs < 1 or gs > 36: msg = bot.reply_to(m, "1-36."); del_both(m, msg.message_id); return
    if uid not in GAME_SESSIONS or GAME_SESSIONS[uid].get("type") != "bingo":
        GAME_SESSIONS[uid] = init_game(uid, "bingo")
    g = GAME_SESSIONS[uid]
    if bt > g["bal"] or bt < 1: msg = bot.reply_to(m, f"So du game: {g['bal']} xu."); del_both(m, msg.message_id); return
    ...
    bao_bonus, is_bao = bao_x10(bt)
    if gs == result: win = bt * 8 + bao_bonus; g["bal"] += win; out = f"TRUNG! +{win} xu" + (" BAO!!!" if is_bao else "")
    else: g["bal"] -= bt; out = f"Thua -{bt} xu"
    brain.stats["games_played"] += 1
    msg = bot.reply_to(m, f"BINGO: So {result}\nDoan: {gs}\n{out} | Game: {g['bal']} xu")
    del_both(m, msg.message_id)

# ─── GAME 12: ĐOÁN SỐ SIÊU TỐC + BÃO X10 ────────────────────────────────
...
def doanso2_cmd(m):
    ...
    if len(parts) < 2:
        ...
        msg = bot.reply_to(m, "DOAN SO SIEU TOC BAO X10\n/doanso2 [so] (1-100)\n5 lan doan!"); del_both(m, msg.message_id); return
    try: gs = int(parts[1])
    except: msg = bot.reply_to(m, "Nhap so 1-100."); del_both(m, msg.message_id); return
    if gs < 1 or gs > 100: msg = bot.reply_to(m, "1-100."); del_both(m, msg.message_id); return
    ...
    if gs == g["secret"]:
        ...
        add_balance(uid, rw)
        msg = bot.reply_to(m, f"CHINH XAC! +{rw} xu" + (" BAO!!!" if is_bao else "")); del GAME_SESSIONS[uid]
    elif g["att"] >= g["max"]: msg = bot.reply_to(m, f"HET LUOT! So la {g['secret']}."); del GAME_SESSIONS[uid]
    elif gs < g["secret"]: msg = bot.reply_to(m, f"CAO HON ({g['max']-g['att']} lan)")
    else: msg = bot.reply_to(m, f"THAP HON ({g['max']-g['att']} lan)")
    del_both(m, msg.message_id)
...
def daily_cmd(m):
    ...
    daily_checkin[uid] = today; rw = 500 + random.randint(0, 1000); add_balance(uid, rw)
    ...
    while True:
        try:
            ...
            if now.minute == 0 and now.hour != last_hour and users:
                ...
                msg = bot.send_message(GROUP_ID, f"{now.strftime('%H:%M')} | {uname}... {random.choice(get_kho())}")
...
def main():
    ...
    logger.info(f"NAO ROBOT 2000 DONG FULL AI KHỞI ĐỘNG")
    ...
    logger.info(f"AI Brain + AI RAM + AI No Hu + AI Cau Do")
